						...
						<Menu.Items className="noOutline absolute right-0 w-56 mt-10 origin-top-right bg-white rounded-md shadow-2xl">
							<div className="px-2 py-2 dark:bg-gray rounded-t-md" >
								<Menu.Item>
									<MyLink 
										href="/" 
										className={`duration-200 hover:text-lightblue ${router.pathname == '/' ? 'text-lightblue' : ''}`}
									>
							...
							<div className="px-2 py-2 dark:bg-gray">
								<Menu.Item>
									<MyLink
										href="/references" 
										className={`duration-200 hover:text-lightblue ${router.pathname == '/references' ? 'text-lightblue' : ''}`}
									>
							...
							<div className="px-2 py-2 dark:bg-gray">
								<Menu.Item>
									<MyLink
										href="/skills" 
										className={`duration-200 hover:text-lightblue ${router.pathname == '/skills' ? 'text-lightblue' : ''}`}
									>
							...
							<div className="px-2 py-2 dark:bg-gray rounded-b-md">
								<Menu.Item>
									<MyLink
										href="/contact" 
										className={`duration-200 hover:text-lightblue ${router.pathname == '/contact' ? 'text-lightblue' : ''}`}
									>
	...
	function Darkmode() {
		...
		return (
		  <div className="text-center">
				<button onClick={() => setTheme('light')} className="hidden dark:block text-white" ><FontAwesomeIcon icon={faSun} /></button>
				<button onClick={() => setTheme('dark')} className="block dark:hidden text-darkgray" ><FontAwesomeIcon icon={faMoon} /></button>
		  </div>
		);
	}
			...
			<div className="grid grid-cols-3 h-20 text-darkgray absolute w-full top-0 dark:bg-darkgray dark:text-white z-50">
				<button className="flex col-span-1 mx-auto my-auto" onClick={() => redirect('/')}>
					<Image 
						...
						layout="fixed"
					/>
					<span className="my-auto md:ml-2 text-3xl font-bold font-roboto">Jannis</span>
				</button>
			...
			<div className='bg-lightgray dark:bg-gray dark:text-white w-full md:p-20 p-10' >
				<div className='lg:w-4/5 max-w-5xl mx-auto' >
					<div className='text-center md:text-xl italic font-[500]' >
						<p>I’m a web developer working with various languages and frameworks. I’m always excited to build new projects and learn new things to extend my knowledge. It’s very fascinating what you can do with programming.</p>
					</div>
